...
1. Can I write Given/When/Then without mentioning class names, method signatures, or dict keys?
...
1. **Create the feature file** — `src/pac/<module>/features/<name>.feature`
1. **Write scenarios** — declarative, no implementation details, business-readable
1. **Review scenarios** — can a non-developer understand what the system should do?
1. **Create step def file** — `src/pac/<module>/tests/test_<name>_bdd.py`
1. **Implement steps** — use `ctx` dict, `parsers.parse()`, real fixtures
1. **Run and verify** — `just test -k test_<name>_bdd`
