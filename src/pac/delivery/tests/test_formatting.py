...
class TestFormatPortfolioStatus:
    def test_all_asset_classes_listed(
        ...
        assert "Stocks ETF" in text
        assert "Gold ETC" in text
        assert "Bond ETF" in text
    ...
    def test_severity_emojis_match_deviations(
        ...
        lines = text.split("\n")
        stocks_line = next(line for line in lines if "Stocks ETF" in line)
        assert "🚨" in stocks_line
        # WARNING gold line should have ⚠️
        gold_line = next(line for line in lines if "Gold ETC" in line)
