...
from pac.analysis.rebalance import calculate_pac_plan
...
class TestCalculatePacPlan:
    def test_basic_redistribution(
        ...
        stocks_alloc = plan.allocations[AssetClass.STOCKS]
        bonds_alloc = plan.allocations[AssetClass.BONDS]
        gold_alloc = plan.allocations[AssetClass.GOLD]
    ...
    def test_overweight_class_gets_zero(
        ...
        assert plan.allocations[AssetClass.STOCKS].amount == Decimal(0)
        # Gold and bonds should split the budget
        assert plan.allocations[AssetClass.GOLD].amount > 0
        assert plan.allocations[AssetClass.BONDS].amount > 0
    ...
    def test_empty_portfolio_distributes_by_target(
        ...
        assert plan.allocations[AssetClass.STOCKS].amount == Decimal("350.00")
        assert plan.allocations[AssetClass.GOLD].amount == Decimal("75.00")
        assert plan.allocations[AssetClass.BONDS].amount == Decimal("75.00")
    ...
    def test_single_underweight_class(
        ...
        assert plan.allocations[AssetClass.BONDS].amount > Decimal(0)
        total = sum(a.amount for a in plan.allocations.values())
        assert total == plan.total_budget
