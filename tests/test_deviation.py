...
class TestGetTargetAllocations:
    def test_returns_decimal_targets(self, default_settings: Settings) -> None:
        ...
        assert targets == {
            AssetClass.STOCKS: Decimal(70),
            AssetClass.GOLD: Decimal(15),
            AssetClass.BONDS: Decimal(15),
        }
    ...
    def test_custom_targets(self) -> None:
        ...
        assert targets[AssetClass.STOCKS] == Decimal(50)
        assert targets[AssetClass.GOLD] == Decimal(30)
        assert targets[AssetClass.BONDS] == Decimal(20)
...
class TestCalculateDeviations:
    ...
    def test_empty_portfolio(self, default_settings: Settings) -> None:
        ...
        assert report.deviations[AssetClass.STOCKS].deviation_pct == Decimal(-70)
        assert report.deviations[AssetClass.GOLD].deviation_pct == Decimal(-15)
        assert report.deviations[AssetClass.BONDS].deviation_pct == Decimal(-15)
    ...
    def test_all_asset_classes_present(
        self,
        sample_snapshot: PortfolioSnapshot,
        default_settings: Settings,
    ) -> None:
        report = calculate_deviations(sample_snapshot, default_settings)
        assert set(report.deviations.keys()) == set(AssetClass)
    ...
    def test_deviation_sign_convention(
        ...
        assert report.deviations[AssetClass.STOCKS].deviation_pct > 0
        assert report.deviations[AssetClass.GOLD].deviation_pct < 0
        assert report.deviations[AssetClass.BONDS].deviation_pct < 0
